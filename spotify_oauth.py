...
input_page = '''
  ...
  <form method="POST" action="/create_playlist">
    <input name="apple_url" style="width:400px"
      placeholder="https://music.apple.com/.../playlist/..." required/>
...
def fetch_apple_playlist(apple_url):
    """Use Playwright to render the page and scrape songs."""
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        page = browser.new_page()
        page.goto(apple_url)
        page.wait_for_selector('div.songs-list-row__song-name', timeout=20000)
        titles  = page.query_selector_all('div.songs-list-row__song-name')
        artists = page.query_selector_all('div.songs-list-row__by-line')
        songs = []
        for t, a in zip(titles, artists):
            songs.append((t.inner_text().strip(), a.inner_text().strip()))
        browser.close()
        return songs
...
def create_playlist():
    ...
    songs = fetch_apple_playlist(apple_url)
    if not songs:
        return "❌ Failed to parse any songs from that Apple Music URL."
